...
class Guest extends Model implements CrudableContract, SearchableContract
{
    ...
    {
        return $this->hasOne(Reservation::class)->where('start_date', '<=', static::getCheckinThresholdDay()->format('Y-m-d'))->latest('start_date')->limit(1);
    }
    ...
    {
        return $this->reservations()->where('start_date', '<=', static::getCheckinThresholdDay()->format('Y-m-d'))->latest('start_date')->limit(5);
    }
}
